...
World of Warcraft addon that saves and restores action bar presets. Players capture their current bar configuration (all 132 slots across bars 1-11) into named presets, then swap between them with one click. Features global and per-character preset scoping, category-based filtering, per-bar filtering, custom icons, layout preservation, confirmation popups for form/dragonriding bars, and a draggable UI with preset editor.
...
**Scanner** (`Engine/Scanner.lua`) reads all 132 slots (11 bars × 12 slots) via `GetActionInfo()`, classifying each as spell/item/macro/mount/toy/flyout/equipmentset/summonpet. Items that are toys are auto-reclassified; legacy `companion` types are resolved to toy or summonpet. Macro entries include an `isCharacter` flag (true/false) to distinguish character-specific macros from account macros, enabling correct restoration when both pools contain same-named entries. Bars 9-11 capture form/dragonriding bar state. `NS.ScanBars()` returns a sparse actions table (only non-empty slots stored). `NS.CountActions()` counts entries for display.
...
- `NS.FindMacroIndex(action)` — resolves macro name to index using `isCharacter` flag. Explicit entries (isCharacter==true/false) search only that pool; legacy entries (isCharacter==nil) fall back to both-pool search account-first, preventing silent substitution when a same-named macro is deleted

**Restore** (`Engine/Restore.lua`) applies presets to bars. `NS.ApplyPreset(preset, skipConfirm)` iterates all 132 slots — first checks per-bar filters (`barFilters`), then checks per-category filters, clears empty ones (unless `preserveLayout` is on), and places actions using type-specific pickup APIs. If `skipConfirm` is falsy and the preset would modify bars 9-11 (form/dragonriding), shows a confirmation popup via `GetAffectedSpecialBars()` heuristic before proceeding. Failed placements retry up to `NS.RETRY_MAX` (5) times at `NS.RETRY_INTERVAL` (0.1s) intervals using `C_Timer.After()`. An async sentinel counter delays the summary chat message until all retries complete.
...
- `barFilters` — per-bar booleans keyed 1-11: controls whether each action bar is restored. Bars 1-8 are regular, 9-11 are form/dragonriding pages
- `actions` — sparse table keyed by slot number (1-132), values are `{type, id}`, `{type="macro", name, isCharacter}` for macros, or `{type="equipmentset", name}` for equipment sets
...
**Bar filters** — Each preset has per-bar filter state (`barFilters[1..11]`). Disabled bars skip all slots in that bar range during restoration. Bar filters are checked before category filters in the restore loop. Legacy presets default bars 1-8 to true and bars 9-11 to false on first load (migration safety: prevents silent form bar modifications on upgrade). New presets default all bars to true.
...
**Macro scope handling** — WoW allows both account-wide (indices 1-120) and character-specific (indices 121-138) macros with overlapping names. Each macro action stores an `isCharacter` flag at scan time: explicit true/false entries search only that pool on restore, preventing silent substitution when a same-named macro is deleted. Legacy presets (isCharacter==nil) fall back to both-pool search for backward compatibility.

**Form bar confirmation** — Bars 9-11 are context-dependent (Bear form, Moonkin/Travel form, Dragonriding) and typically not visible. `ApplyPreset()` checks if the preset would actually modify these bars via `GetAffectedSpecialBars()` (checks both preset data and current bar state to avoid false positives), and if so, shows a `StaticPopupDialog` confirmation before proceeding. The popup captures preset identity (scope, name) rather than a direct reference, allowing re-resolution at confirm time to handle deletion/rename races and scope switches mid-dialog.
