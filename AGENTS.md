...
- `barFilters` — per-bar booleans keyed 1-15: controls whether each action bar is restored. Bars 1-6 are the standard visible action bars (Main + 4 MultiBars, slots 1-72). Bars 7-10 (slots 73-120) are class form/stance pages — Druid Bear=7, Cat=8, Moonkin=9, Travel=10; Rogue Stealth=7; Warrior stances=7-9; Evoker bonus=7 — *paged onto Bar 1 when in form*, not visible as separate bars in Blizzard's UI. Bar 11 (slots 121-132) is Dragonriding/Skyriding override / vehicle / possess. Bar 12 (slots 133-144) is reserved/unused. Bars 13-15 (slots 145-180) are Blizzard's MultiBar5/6/7 — visible as **Action Bar 6, 7, 8** in the Action Bars options panel. See `NS.BAR_TOOLTIPS` in Constants.lua for per-bar hover text.
...
**Special bar confirmation** — Bars 7-12 are class form/stance pages (Druid Bear=7, Cat=8, Moonkin=9, Travel=10; Rogue Stealth=7; Warrior stances=7-9; Evoker bonus=7) plus Dragonriding/override (11) and reserved page 12. These are paged onto Bar 1 dynamically and aren't visible as separate bars in the Blizzard UI. `NS.RequestApplyPreset()` (UI layer) checks if the preset would actually modify any of these bars via `GetAffectedSpecialBars()` (checks both preset data and current bar state to avoid false positives) and shows a `StaticPopupDialog` confirmation before proceeding. Bars 13-15 (Action Bar 6-8 in Blizzard's UI) are NOT in the popup — they're regular user-facing bars. The popup captures preset identity (scope, name) rather than a direct reference, allowing re-resolution at confirm time to handle deletion/rename races and scope switches mid-dialog.
